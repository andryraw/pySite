...
from app.forms import RegistrationForm, LoginForm, EditForm, \
    ...
    AddGenre, AddDirector, AddFilmPosterForm, \
    DeleteFilmPosterForm
...
def film_info(id):
    ...
    if request.method == "POST":
        ...
        if form_del.validate_on_submit():  # delete poster button
            ...
            db.session.commit()
            return redirect(url_for('film_info', id=id))
    return render_template('film_info.html', title=film, form=form, form_del=form_del, film=film)
